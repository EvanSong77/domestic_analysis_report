...
class ConcurrencyConfig(BaseModel):
    """并发控制配置"""
    max_total_concurrent: int = Field(default=2, ge=1, description="所有实例允许的最大并发数")
    lock_file_path: str = Field(default="/tmp/concurrency_lock.json", description="锁文件路径")
    acquire_timeout: int = Field(default=30, gt=0, description="获取锁的超时时间（秒）")
    instance_heartbeat_interval: int = Field(default=60, gt=0, description="实例心跳间隔（秒）")
    stale_instance_timeout: int = Field(default=300, gt=0, description="实例超时时间（秒）")
...
class Settings(BaseSettings):
    ...
    # 并发控制配置
    concurrency: ConcurrencyConfig = Field(default_factory=ConcurrencyConfig, description="并发控制配置")
