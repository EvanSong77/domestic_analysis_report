...
class AIModelService:
    ...
    async def _generate_concurrent_reports_by_content_type(self, expanded_tasks: List[Dict], input_count: int) -> Dict:
        ...
        try:
            ...
            # 将每个 origin_index 的所有内容类型拼接成最终报告（保持输入顺序）
            final_reports = []
            for origin_index in range(0, input_count):
                ...
                if self.settings.use_fix_tag:
                    ...
                    # 调用独立的异步修复方法
                    cur_content, cum_content, cur_validate_result, cum_validate_result = await self._async_fix_tags(
                        cur_content, cum_content, cur_template, cum_template
                    )

                    if not cur_validate_result['is_valid'] or not cum_validate_result['is_valid']:
                        combined_content = cur_content + cum_content

                # 构建状态信息字符串
                status_str = ", ".join([f"{ct}: {status}" for ct, status in status_info.items()])

                # 如果所有板块都没有成功（注意 no_data 被视作非 success）
                if not has_success:
    ...
    async def _async_fix_tags(self, cur_content: str, cum_content: str, cur_template: str, cum_template: str) -> tuple:
        """
        异步修复标签的独立方法
        参数：
            cur_content: 当前内容
        ...
        async def fix_content(content, template, validate_result):
            if not validate_result['is_valid']:
                ...
                if fix_result.get('status') == 'success':
                    ...
                    logger.info(f"修复结果: {fix_contents}")
