...
class AsyncDataQueryService:
    ...
    async def _expand_by_modules_provinces_offices(self, period: str, modules: List[str],
                                                  provinces: List[str], timeout: float = 30.0) -> List[Dict]:
        """异步展开：模块 × 省份 × 二级办"""
        results = []
        tasks = []
    ...
    async def _execute_split_queries(self, request: DiagnosisRequest, timeout: float = 60.0) -> List[Dict]:
        ...
        if is_module_all and is_province_all and is_office_all:
            ...
            return await self._expand_by_modules_provinces_offices(period, modules, provinces, timeout)
        
        # 场景2: 模块=ALL/空 + 省份=ALL + 二级办=空/具体值
        if is_module_all and is_province_all:
            provinces = await self._get_provinces(period, timeout)
            return await self._expand_by_modules_provinces(period, modules, provinces, office_lv2_name)
        ...
        if is_module_all and is_office_all:
            ...
            return await self._expand_by_modules_offices(period, modules, province_name, offices)
        
        # 场景4: 模块=ALL/空 + 省份和二级办都是具体值/空
        if is_module_all:
            return await self._expand_by_modules(period, modules, province_name, office_lv2_name)
        
        # 场景5: 模块=具体值 + 省份=ALL + 二级办=ALL
        if is_province_all and is_office_all:
            provinces = await self._get_provinces(period, timeout)
            return await self._expand_by_provinces_offices(period, diagnosis_type, provinces, timeout)
        
        # 场景6: 模块=具体值 + 省份=ALL + 二级办=空/具体值
        if is_province_all:
            provinces = await self._get_provinces(period, timeout)
            return await self._expand_by_provinces(period, diagnosis_type, provinces, office_lv2_name)
        ...
        if is_office_all:
            if not province_name:
                raise ValueError("二级办为ALL时，省区参数不能为空")
            offices = await self._get_offices(period, province_name, timeout)
            return await self._expand_by_offices(period, diagnosis_type, province_name, offices)
