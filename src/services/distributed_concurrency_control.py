...
class DistributedConcurrencyControl:
    """
    分布式并发控制服务 - 多实例协调控制（改进版）

    ⭐ 核心改进：
    1. **使用内存锁 + 文件锁结合方案**（避免文件锁竞争）
    2. **异步友好的锁获取**（非阻塞式 poll）
    3. **文件操作超时保护**（防止卡死）
    4. **自动清理机制优化**（更激进的过期实例清理）
    5. **独立的锁文件操作线程**（不阻塞主线程）
    """
    ...
    def __init__(self, config=None, max_total_concurrent: int = None, lock_file_path: str = None):
        ...
        self._last_file_write_time = 0
        self._file_write_interval = 0.5  # 最多每 0.5 秒写一次文件
    ...
    def _initialize_lock_file(self):
        ...
        try:
            ...
            # 同步内存状态
            self._memory_state = initial_state
            self._last_file_write_time = time.time()
    ...
    def _acquire_file_lock(self, timeout: float = 2.0) -> bool:
        ...
        retry_count = 0
        max_retries = int(timeout * 10)  # 100ms 重试一次
        ...
        while time.time() - start_time < timeout and retry_count < max_retries:
            try:
                ...
                logger.debug(f"获取文件锁异常（重试{retry_count}）: {e}")
                retry_count += 1
                if self.lock_file:
                    try:
                        self.lock_file.close()
        ...
        logger.warning(
            f"文件锁获取超时（{timeout}秒内，重试{retry_count}次）- "
            f"可能存在锁竞争或死锁，考虑增加超时时间或减少并发数"
        )
        return False
    ...
    def _write_lock_data(self, data: dict) -> bool:
        """
        写入锁文件数据（改进版 - 使用 debounce 机制减少文件 I/O）
        ...
        try:
            ...
            if current_time - self._last_file_write_time < self._file_write_interval:
                # 只更新内存，不写文件
                self._memory_state = data.copy()
                return True
            ...
            # ⭐ 更新内存缓存和最后写入时间
            self._memory_state = data.copy()
            self._last_file_write_time = current_time
    ...
    def _cleanup_stale_instances(self, lock_data: dict, timeout: Optional[int] = None) -> dict:
        ...
        for instance_id, instance_info in active_instances.items():
            ...
            if age < timeout:
                ...
                logger.warning(
                    f"清理过期实例 - id: {instance_id}, 年龄: {age:.1f}s(>={timeout}s), "
    ...
    async def acquire_slot(
        ...
        while time.time() - start_time < timeout:
            # ⭐ 使用较短的锁获取超时（1 秒）
            if not self._acquire_file_lock(timeout=1.0):
                # 如果获取失败，异步等待后重试
                ...
                if current_concurrent < max_concurrent:
                    ...
                    if self._write_lock_data(lock_data):
                        logger.info(
                            ...
                            f"owner: {owner_id}, 重试次数: {poll_count}"
                        )
                        return True
                    else:
                    ...
                    # 无可用槽位，记录占用情况
                    active_instances = lock_data.get('active_instances', {})
                    if poll_count == 0:  # 第一次打印
                        logger.warning(
                            f"⏳ 无可用槽位 - 当前: {current_concurrent}/{max_concurrent}, "
                            f"活跃实例数: {len(active_instances)}, 将继续等待..."
                        )
        ...
        logger.warning(
            f"获取并发槽位失败/超时 - instance_id: {instance_id}, "
            f"超时时间: {timeout}秒, 总重试次数: {poll_count}"
        )
        return False
    ...
    async def release_slot(self, instance_id: str) -> bool:
        ...
        try:
            ...
            if self._write_lock_data(lock_data):
                ...
                logger.error(f"保存锁文件失败（槽位释放可能不完整） - instance_id: {instance_id}")
                return False

        except Exception as e:
            logger.error(f"释放并发槽位异常 - instance_id: {instance_id}, 错误: {e}")
            return False
        finally:
            self._release_file_lock()
    ...
    async def get_current_status(self, include_details: bool = False) -> dict:
        ...
        # ⭐ 优先从内存缓存获取（快速路径）
        if self._memory_state:
            lock_data = self._memory_state.copy()
        else:
            if not self._acquire_file_lock(timeout=1.0):
                logger.warning("无法获取文件锁，返回空状态")
                return {'error': '无法获取并发状态'}
