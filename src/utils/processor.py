...
from src.config.constants import CONTENT_WHERE_DICTS, NONE_VALUE_KEY, CONTENT_TYPE_CONFIG, SPECIAL_PROVINCES_SQL
...
class DataTemplateProcessor:
    """新的数据处理器 - 支持两种内容类型(CURRENT, CUMULATIVE)和独立SQL"""
    ...
    def _execute_queries(self, sql: str, params: Dict, data_type: str):
        """拼接sql并查询数据"""
    ...
    def process(self, params: Dict) -> Dict[str, Dict]:
        ...
        for content_type in dimension_content_types:
            ...
            for rule in rules:
                ...
                content_data = self._execute_queries(sql, params, use_data_type if use_data_type else CONTENT_WHERE_DICTS[content_type])
