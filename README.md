...
### 4. 高可靠的并发控制
- 分布式并发控制，支持最大并发数配置（max_total_concurrent）
- 心跳保活机制，防止任务被误清理（instance_heartbeat_interval）
- 严格的任务顺序执行，确保数据一致性
- 任务完全执行后才释放资源，避免资源竞争
- 支持实例超时清理（stale_instance_timeout）
...
```json
{
  ...
  "data": {
    ...
    "active_instances_details": {
      "instance_12345678-1234-5678-1234-567812345678": {
        "owner_id": "202510151603000",
        "concurrent_count": 1,
        "acquire_time": 1734326400,
        "last_heartbeat": 1734326415,
        "meta": {
          "owner_id": "202510151603000",
          "acquire_time": 1734326400,
...
#### 7. 并发控制配置
- **并发限制**：max_total_concurrent
- **文件路径**：lock_file_path
- **超时配置**：acquire_timeout、stale_instance_timeout
- **心跳配置**：instance_heartbeat_interval
...
concurrency:
  ...
  lock_file_path: "logs/concurrency_lock.json"  # 锁文件路径
  acquire_timeout: 5  # 获取锁的超时时间（秒）
  instance_heartbeat_interval: 60  # 实例心跳间隔（秒）
  stale_instance_timeout: 300  # 实例超时时间（秒）
```
